...
to ant-move
  ...
  [
    ...
    [;;else;;
      ifelse (abs(xcor) > max-pxcor - 2.5) or (abs(ycor) > max-pycor - 2.5) ;;ant is off the world.
      [
        die
      ]
      [;;else;;
...
to pick-a-patch-returned
  ...
  ;;can use threshcount to clear some bias.
  if (threshcount = 2)
  [
    set p_l 0.5
    set p_r 0.5
  ]
  ;;first let's try to just get these dudes to sit closely to the stream.
  set x ( (random 100) / 100)
  ifelse (x < p_l)
  [
    ;;do a left turn
